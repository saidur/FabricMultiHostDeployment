...
app.get('/api/queryallcars', async function (req, res) {
    try {
        ...
        const identity = await wallet.get('saidur');
        if (!identity) {
        ...
        await gateway.connect(ccp, { wallet, identity: 'saidur', discovery: { enabled: true, asLocalhost: false } });
        // Get the network (channel) our contract is deployed to.
        const network = await gateway.getNetwork('mychannel');

        //console.log (' hello wroldd.....');
        ...
        const result = await contract.submitTransaction('createCar', '2234234', 'aus','test', 'achor', 'saidur');
...
app.get('/api/query/:car_index', async function (req, res) {
    try {
        ...
        const identity = await wallet.get('saidur');
        if (!identity) {
            console.log('An identity for the user "saidur" does not exist in the wallet');
            console.log('Run the registerUser.js application before retrying');
            return;
        }
...
app.post('/api/addcar/', async function (req, res) {
    try {
       ...
       await contract.submitTransaction('createCar', '2234234', 'aus','test', 'achor', 'saidur');
